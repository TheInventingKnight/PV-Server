...
class cl_methods:
    ...
    async def gvar(self, client, message, listener):
        ...
        for room in rooms:
            print(message["name"])
            if "Account Data" in message["name"]:
              print("ho")
              if "Account Data e" == message["name"]:
                print("E command")
                print("Put more code  in cl methods line 367!")
                open('Saved_Data.txt', 'w').close()
              elif "Account Data fix" == message["name"]:
                print ("FIX command")
                with open('Saved_Data.txt', 'rb+') as filehandle:
                    filehandle.seek(-2, os.SEEK_END)
                    filehandle.truncate()
              else:
                Save_Data(message["val"])
