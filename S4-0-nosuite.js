...
class CloudLink {
	...
	runCMDnoID({CMD, DATA}) {
		const self = this;
		if (this.isRunning) {
			if (!(String(CMD).length > 100) || !(String(DATA).length > 3000)) {
				let tmp_msg = {
					cmd: String(CMD),
					val: autoConvert(DATA)
				}
	...
	runCMD({CMD, ID, DATA}) {
		const self = this;
		if (this.isRunning) {
			if (!(String(CMD).length > 100) || !(String(ID).length > 20) || !(String(DATA).length > 3000)) {
				let tmp_msg = {
					cmd: String(CMD),
					id: autoConvert(ID),
					val: autoConvert(DATA)
